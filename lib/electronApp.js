...
import { installDependencies, getElectronVersion } from 'electron-builder/out/util/util';
...
export default class ElectronApp {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (this.$.utils.exists(this.$.env.paths.electronApp.nodeModules)) {
                ...
                spawn(npm, ['prune'], {
                    ...
                    this.log.verbose('executing npm install');
                    spawn(npm, ['install'], {
                        cwd: this.$.env.paths.electronApp.root,
     ...
     */
    async rebuildDeps() {
        if (!this.$.desktop.getSettings().rebuildBinaryPackages) {
            this.log.warn('native modules rebuild is turned of, be sure to turn it on if you added any native node ' +
                'modules');
            return;
        }

        this.log.info('issuing native modules rebuild from electron-builder');
        // Since electron-rebuild was causing more problems than giving benefits
        // we will use rebuilding from electron-builder.
        // Apparently it is not a big deal to invoke it.
        // TODO: implement something like shouldRebuildNativeModules
        ...
        await installDependencies(
            this.$.env.paths.electronApp.root, results, 'x64', true, 'rebuild');

        /*
         this.log.info('checking if native modules should be rebuilt');
         ...
         this.log.verbose('ensuring correct node headers');

...
export default class ElectronApp {
    ...
    async transpileAndMinify() {
        ...
        const preset = presetEnv({ assertVersion: () => { } }, { targets: { node: '12' } });
            ...
            data: files } = await this.$.utils.readDir(this.$.env.paths.desktopTmp.root);
...
/**
    oves all the files that should not be packed into asar into a safe location which is the
    ...
    shell.mkdir(this.$.env.paths.electronApp.extracted);
        ...
        t configs = this.$.desktop.gatherModuleConfigs();
        
            files that should not be asar'ed.
                ach((config) => {
            t moduleConfig = config;
            'extract' in moduleConfig) {
                !Array.isArray(moduleConfig.extract)) {
                moduleConfig.extract = [moduleConfig.extract];
                    
                leConfig.extract.forEach((file) => {
                this.log.debug(`excluding ${file} from ${config.name}`);
                    t filePath = path.join(

...
import { getProductionDependencies, createLazyProductionDeps } from 'electron-builder-lib/out/util/packageDependencies';
...
export default class InstallerBuilder {
    ...
    wait() {
        ...
        const platformDir = `win-${this.currentContext.arch === 'ia32' ? 'ia32-' : ''}unpacked`;
        const appAsarPath = path.join(
            this.$.env.options.output,
            this.$.env.paths.installerDir,
            platformDir,
        ...
        return new Promise((resolve, reject) => {
            function check() {
                fs.open(appAsarPath, 'r+', (err, fd) => {
                    retries += 1;
                    if (err && 'code' in err && err.code === 'EBUSY') {
                        if (retries < 6) { setTimeout(() => check(), 500); } else {
    ...
    moveNodeModulesOut() {
        return new Promise((resolve, reject) => {
            this.log.debug('moving node_modules out, because we have them already in' +
                ' app.asar');
            this.removeDir('-rf', this.$.env.paths.electronApp.tmpNodeModules)
                .catch(e => reject(e))
                .then(() => {
                    shell.config.fatal = true;
                    ...
                    try {
                        ...
                        return this.removeDir(this.$.env.paths.electronApp.nodeModules);
                    } catch (e) {
                        shell.config.reset();

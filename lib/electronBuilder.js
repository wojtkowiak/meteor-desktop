...
export default class InstallerBuilder {
    ...
    async installOrRebuild(arch, platform = process.platform) {
        ...
        const results = await getElectronVersion(this.$.env.paths.meteorApp.root,
            devMetadata);
        const productionDeps = createLazyProductionDeps(this.$.env.paths.meteorApp.root);
        this.lastRebuild = {
            frameworkInfo: { version: results, useCustomDist: false },
            platform,
            arch,
            productionDeps
        };
        await installOrRebuild(this.$.desktop.getSettings().builderOptions || {},
            this.$.env.paths.electronApp.root, this.lastRebuild);
    }

...
export default class MeteorApp {
    ...
    async acquireIndex() {
        ...
        const url = this.$.env.options.ddpUrl || `http://127.0.0.1:${port}`;
        this.log.info('acquiring index.html');
        const res = await fetch(`${url}/__cordova/index.html`);
    ...
    async acquireManifest() {
        ...
        const res = await fetch(
            `${url}/__cordova/manifest.json?meteor_dont_serve_index=true`
        );
        const text = await res.text();

...
export default class MeteorApp {
    ...
    async ensurePackages() {
        if (this.$.desktop.getSettings().desktopHCP) {
            ...
            if (!this.checkPackages(packages)) {
                this.log.warn('desktopHCP is set but no required packages are added to project, will try to add them now');
                await this.addPackages(packages, packagesWithVersion);
            }
            if (!this.checkPackagesVersion(packagesWithVersion)) {
                this.log.warn('desktopHCP is set but required packages are not at the same version than current meteor-desktop version, fixing it');
                await this.addPackages(packages, packagesWithVersion);
            }
        }
    }
    ...
    updateGitIgnore() {
        ...
        if (!~gitIgnore.indexOf(this.$.env.paths.electronApp.rootName)) {
            this.log.info(`adding ${this.$.env.paths.electronApp.rootName} to .meteor/.gitignore`);
    ...
    isCordovaBuildReady() {
        ...
        return fs.existsSync(this.$.env.paths.meteorApp.webCordovaProgramJson) &&
            (
                !this.oldManifest ||
                (this.oldManifest &&
                    this.oldManifest !== fs.readFileSync(
    ...
    async acquireIndex() {
        const port = (this.$.env.options.meteorPort) ? this.$.env.options.meteorPort : 3080;
    ...
    updateDdpUrl() {
        ...
        try {
            content = fs.readFileSync(this.$.env.paths.meteorApp.cordovaBuildIndex, 'UTF-8');
        } catch (e) {
        ...
        try {
            fs.writeFileSync(this.$.env.paths.meteorApp.cordovaBuildIndex, content);
        } catch (e) {
    ...
    prepareArguments() {
        ...
        if (this.$.env.options.meteorPort) {
            args.push(this.$.env.options.meteorPort);
        } else {
            args.push('3080');
        }
        return args;
    ...
    async copyBuild() {
        ...
        if (!fs.existsSync(cordovaBuild)) {
            this.log.error(`no cordova build found at ${cordovaBuild}`);
            this.log.error('run without -s to perform a new meteor build');
            throw new Error('required file not present');
        }

        if (!fs.existsSync(cordovaBuildProgramJson)) {
            this.log.error('no program.json found in cordova build found at ' +
                `${cordovaBuild}`);
            this.log.error('run without -s to perform a new meteor build');
            throw new Error('required file not present');
        }
        ...
        if (this.indexHTMLstrategy !== this.indexHTMLStrategies.INDEX_FROM_RUNNING_SERVER) {
            if (!fs.existsSync(cordovaBuildIndex)) {

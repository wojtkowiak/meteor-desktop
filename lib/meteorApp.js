    ...
    async ensurePackages() {
        if (this.$.desktop.getSettings().desktopHCP) {
            ...
            if (!this.checkPackages(packages)) {
                this.log.warn('desktopHCP is set but required packages are added to project, will try to add them now');
                try {
            ...
            if (!this.checkPackagesVersion(packagesWithVersion)) {
                this.log.warn('desktopHCP is set but required packages version is not equal ' +
                    'to meteor-desktop version, fixing it');
                try {
                    await this.addPackages(packages, packagesWithVersion);
                } catch (e) {
                    throw new Error(e);

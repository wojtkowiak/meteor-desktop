...
class MeteorDesktop {
     ...
     * @constructor
     */
    constructor(input, output, runFromDist, ddpUrl) {
        this.log = new Log('index');

        console.log(1, ddpUrl);

        this.log.info('initializing', ddpUrl);

        this.env = require('./env')(input, output, ddpUrl);
...
module.exports = function exports(input, output, settings, ddpUrl) {
    return new MeteorDesktop(input, output, settings, ddpUrl);
};

...
if (!('desktopHCP' in settings) || !settings.desktopHCP) {
...
} else if ('skadmin:meteor-desktop-bundler' in Package) {
    const chokidar = Npm.require('chokidar');
    const versionFile = path.join(rootPath, 'version.desktop');

    const version =
        typeof Package['skadmin:meteor-desktop-bundler'].METEOR_DESKTOP_VERSION === 'object' ?
            Package['skadmin:meteor-desktop-bundler'].METEOR_DESKTOP_VERSION.version : null;
    ...
    if (version) {
        try {
            ...
            throw new Error('[meteor-desktop] there is no version.desktop file. Are you sure you ' +
                'have skadmin:meteor-desktop-bundler package added to your project?');
        }
    ...
    throw new Error('[meteor-desktop] bundler plugin was not detected. Are you sure you have ' +
        'skadmin:meteor-desktop-bundler package added to your project?');
}
